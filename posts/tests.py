...
class ProfileTest(TestCase):
    ...
    def test_new(self):
        ...
        """
        self.client.force_login(self.user)
        response = self.client.post("/new/", {"text": self.post.text}, follow=True)
        self.assertEqual(response.status_code, 200)
...
class ImageTest(TestCase):
    ...
    def setUp(self):
        ...
        self.client = Client()
        self.user = User.objects.create_user(username='sarah')
        self.group = Group.objects.create(title='foo', slug='foo', description='foo')
        self.post = Post.objects.create(text="test1", author=self.user)
        small_gif = (
        ...
        with ImageFile(image) as fp:
            self.client.post('/sarah/1/edit', {'text': self.post.text, 'group': self.group.id, 'image': fp})
    ...
    def test_image3(self):
        ...
        with open('manage.py', 'rb') as fp:
            self.client.post('/sarah/2/edit', {'text': self.post.text, 'image': fp})
